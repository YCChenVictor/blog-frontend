...
* Visitor can post `email` and `password` to signup
* User can post `email` and `password` to `authenticate`
...
class SessionsController < ApplicationController
  def signup
    user = User.new(email: param[:email], password_digest: password[:password])
    if user.save
