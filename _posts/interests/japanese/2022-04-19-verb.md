...
function te_form(verb) {
  ...
  } else if (['し'].include verb_without_masu(verb)[-1]) {
    return  verb_without_masu(verb) + 'て'
  }
}
...
function from_i_to_a(verb) {
  verb_without_masu(verb) - last_letter + a_initials_of_last_letter
}
```
...
I should refactor the functions above one day
